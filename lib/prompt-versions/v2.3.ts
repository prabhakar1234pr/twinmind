/**
 * Suggestion prompt v2.3.0 — CLARIFYING_INFO "bring it up by saying" rule
 *
 * Change from v2.2.0:
 *   - The CLARIFYING_INFO type definition now mandates the fullContext include
 *     the exact sentence the reader can say to raise the clarification without
 *     derailing, e.g.: "Bring it up by saying: 'Quick aside — X means Y,
 *     which matters because Z.'"
 *
 * Intent: CLARIFYING_INFO was consistently the weakest-scoring type on
 * actionability (often 0/3, judge: "only defines, no action step"). Giving
 * the model an explicit shape for actionability on this type.
 *
 * Eval scores (15 batches):
 *   Total: 14.98 / 18  (-0.15 vs v2.2)
 *   Specificity: 2.91 ↑ ← NEW HIGH until v2.5
 *   Actionability: 2.27 | Preview: 2.49 | Timing: 2.64 ↓ | Meeting-type: 2.47
 *   Variety: 2.20
 *   Summary: 10.80 / 12 (grounding 3/3, structure 3/3)
 *
 * Tradeoff: timing fit dropped 0.18 — forcing CLARIFYING_INFO to produce a
 * rehearsed sentence made some suggestions feel less "in the moment."
 *
 * Raw report: scripts/eval-report-v2.3.0.md
 */

export const SUGGESTION_PROMPT_V2_3 = `You are TwinMind's live meeting copilot, sitting on the shoulder of the person reading this. A conversation is happening RIGHT NOW. You have roughly 30 seconds to put 3 genuinely useful things in front of them.
