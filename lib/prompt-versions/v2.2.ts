/**
 * Suggestion prompt v2.2.0 — mandatory verbatim quote + preview grounding
 *
 * Changes from v2.1.0:
 *   - fullContext part 1 is now a MANDATORY verbatim quote (5-15 words,
 *     character-for-character from foreground, in double quotes). Previously
 *     "the exact quote" but not enforced as non-optional.
 *   - New "Preview grounding" section: every preview must reference a specific
 *     noun from the transcript (named person, company, number, date, concept).
 *
 * Intent: lift Specificity toward 3/3 by making transcript anchoring non-optional.
 *
 * Eval scores (15 batches):
 *   Total: 15.13 / 18  (-0.11 vs v2.1)
 *   Specificity: 2.80 (-0.04 — did NOT lift as predicted)
 *   Actionability: 2.33 | Preview: 2.49 | Timing: 2.82 | Meeting-type: 2.49
 *   Variety: 2.20
 *   Summary: 10.80 / 12 (grounding 3/3, structure 3/3)
 *
 * Why it didn't work: the v2.1 prompt already produced verbatim quotes most of
 * the time; making it "mandatory" added constraint without changing behavior.
 *
 * Raw report: scripts/eval-report-v2.2.0.md
 */

export const SUGGESTION_PROMPT_V2_2 = `You are TwinMind's live meeting copilot, sitting on the shoulder of the person reading this. A conversation is happening RIGHT NOW. You have roughly 30 seconds to put 3 genuinely useful things in front of them.
